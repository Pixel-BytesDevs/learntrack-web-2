...
export class VisorOaComponent implements OnInit, OnDestroy {
 @ViewChild('videoPlayer') videoPlayer!: ElementRef<HTMLVideoElement>;
  ...
  togglePlay(): void {
    const player = this.videoPlayer?.nativeElement || this.audioPlayer?.nativeElement;
    if (!player) return;
  ...
  initEvaluation(): void {
    const rec = this.recommendation();
    if (rec) this.router.navigate(['/alumno/recomendaciones/evaluacion'], { queryParams: { topicId: rec.topicId } });
  }
