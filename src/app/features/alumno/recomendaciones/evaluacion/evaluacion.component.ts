...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, KatexDirective, DecimalPipe, NzButtonModule, NzIconModule, NzProgressModule, NzSpinModule, NzModalModule, NzTagComponent],
...
export class EvaluacionComponent implements OnInit, OnDestroy {
  ...
  currentQuestion = computed(() => {
    ...
    return test ? test.questionTestResponses[this.currentQuestionIndex()] : null;
  });

  progressPercentage = computed(() => {
    const test = this.placementTest();
    if (!test) return 0;
    return ((this.currentQuestionIndex() + 1) / test.questionTestResponses.length) * 100;
  });
  ...
  fetchEvaluation(userId: number, topicId: string): void {
    ...
    this.evaluationService.evaluateRecommendation({ userId, topicId })
  ...
  selectAlternative(alt: AlternativePlacement): void {
    ...
    if (!question) return;
    question.alternatives.forEach(a => a.selected = false);
    alt.selected = true;
  }

  nextQuestion(): void {
    const test = this.placementTest();
    if (test && this.currentQuestionIndex() < test.questionTestResponses.length - 1) {
  ...
  async finishEvaluation(): Promise<void> {
    ...
    const realDuration = Math.floor((test.duration * 60 - this.timeRemaining()) / 60);
    const finalPayload: EvaluationResponse = { ...test, duration: realDuration };
    const userId = Number(this.tokenService.getUsername()) || 1;

    try {
      const result = await this.evaluationService.submitPlacementTest(userId, finalPayload).toPromise();
